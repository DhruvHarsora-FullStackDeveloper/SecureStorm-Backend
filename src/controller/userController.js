...
import { sendWelcomeEmail, sendCancelationEmail } from "../email/sendEmail.js";
...
const userLogin = async (req, res, next) => {
	try {
		...
		if (!user) {
			return res.status(msg.NOT_FOUND_CODE).send({ result: 0, error: msg.USER_NOT_FOUND });
		} else {
...
const userUpdate = async (req, res, next) => {
	try {
		...
		if (!isValidUpdates) {
			return res.status(msg.BAD_REQ_CODE).send({ result: 0, error: msg.INVALID_UPDATES });
		}
...
export { userSignUp, userLogin, userUpdate, updateMasterKey, updatePassword, deleteUser };
